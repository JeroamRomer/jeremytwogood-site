...
//   out-base defaults to the input path minus its extension (so a .png input
//   with no out-base argument is replaced in place). The script appends .png
//   or .jpg depending on which format wins.
...
try {
  ...
  console.error(`compress-thumb: ${err.message}`);
  process.exit(1);
}
